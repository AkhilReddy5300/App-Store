...
class AppStore extends Component {
  ...
  onFilter = searchRes => {
    ...
    const filterApps = searchRes.filter(each => each.category === activeId)
    return filterApps
  }
  ...
  getSearchResults = () => {
    const {searchInput} = this.state
    const searchRes = appsList.filter(each =>
  ...
  render() {
    ...
    const searchRes = this.getSearchResults()
    const filterItems = this.onFilter(searchRes)
    return (
